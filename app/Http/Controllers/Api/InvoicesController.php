...
/*
	...
	{ 
		$requestData = $request->all();
		$result = Invoice::with('items')->where('id', $requestData['id'])->first();
	...
	{ 
		$requestData = $request->all();
		$result = Invoice::with('items')->with('currency')->where('id', $requestData['id'])->get();
		
		if($result){
			return response()->json(['data' => $result,"draw" => $requestData['draw'],"recordsTotal" => count
			($result),"recordsFiltered" => count($result)], 200);
		}else{
			return response()->json(['error' => []], 200);
		}
	}
